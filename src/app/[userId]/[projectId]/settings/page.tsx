...
export default function SettingsPage() {
  ...
  const UpdateSettings = async () => {
    const toastId = toast.loading("Updating project settings...");
    try {
      const res = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/projects/${projectData?.id}`,
        {
          name: projectName,
  ...
  function copyToClipboard(text: string | undefined) {
    if (text) {
      ...
      toast.success("Copied to clipboard!", {
        description:
          text.length > 30 ? `${text.slice(0, 15)}...${text.slice(-15)}` : text,
        action: {
          label: "Close",
